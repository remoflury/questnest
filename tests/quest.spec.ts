...
describe('groups', () => {
	...
	test('create questboard and add, toggle, edit quests', async ({ page }) => {
		...
		await test.step('toggle quest', async () => {
			...
			const form1IsDone = await form1.evaluate((element) =>
				element.classList.contains('bg-primary')
			);
			expect(form1IsDone).toBeTruthy();
			const form2IsDone = await form2.evaluate((element) =>
				element.classList.contains('bg-primary')
			);
			...
			const form1IsUndone = await form1.evaluate(
				(element) => !element.classList.contains('bg-primary')
			);
			expect(form1IsUndone).toBeTruthy();
			const form2IsUnDone = await form2.evaluate(
				(element) => !element.classList.contains('bg-primary')
			);
